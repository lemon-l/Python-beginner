...
class google():
    ...
    def translate(self, word):
        ...
        res = requests.get(self.url.format(
            target_language, self.getTk(word), word), headers=self.headers)
...
class Demo(QWidget):
    def __init__(self, parent=None):
        super().__init__()
        self.setWindowTitle('翻译软件-公众号: Ahab杂货铺')
        ...
        self.resize(400, 150)
        self.translateButton1.clicked.connect(
            lambda: self.translate(api='baidu'))
        self.translateButton2.clicked.connect(
            lambda: self.translate(api='youdao'))
        self.translateButton3.clicked.connect(
            lambda: self.translate(api='google'))
    ...
    def translate(self, api='baidu'):
        ...
        if api == 'baidu':
            ...
            raise RuntimeError(
                'Api should be <baidu> or <youdao> or <google>...')
